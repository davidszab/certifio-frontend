...
function typeConverter(type: string | React.ComponentType) {
	...
	if (index != -1) return customElements[index].component;
	return type;
}

//todo: change any
function templateToComponent(
	{ type, props, children }: Template,
	index?: string,
	ctx?: {}
): any {
	...
	if (Array.isArray(children))
		return React.createElement(
			type,
			{ ...props, key: index, context: ctx },
			children.map((t, i) => {
				return templateToComponent(t, i.toString(), ctx);
			})
		);
		...
		return React.createElement(
			type,
			{ ...props, key: index, context: ctx },
			Handlebars.compile(children)(ctx)
		);
	else if (typeof children == "object")
		return React.createElement(
			type,
			{ ...props, key: index, context: ctx },
			templateToComponent(children, "0", ctx)
		);
	else if (!children)
